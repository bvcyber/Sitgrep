...
_NOTE_: This feature may take several hours to complete, depending on how many findings. You will want to keep the device from sleeping during this to prevent the agent from hanging.
...
- Install works, Sitgrep returns with errors:
  - A case with Opengrep using too much memory can cause Sitgrep to fail. The cause of this is due to using a generic rules having excessively broad pattern matching using the `generic` language type. The solution is to specify the exact supported languages.
